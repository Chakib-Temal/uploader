...
api.user.logout = (cb) => {
    ...
    ipcRenderer.send('write-user-data', '');
    return cb(null);
    ...
    async.series([getPersonalSensorList.bind(null, sessionInfo), getPeriodList], (err, results) => {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (data[i].type == 'smbg') {
                ...
                value = data[i].value.toFixed(3);
            } else if (data[i].type == 'basal' && (data[i].deliveryType == 'scheduled' || data[i].deliveryType == 'automated' )) {
                typeId = "MEASURE_TYPE_BASAL_INSULIN";
                value = data[i].rate.toFixed(3);
            } else if (data[i].type == 'bolus' && (data[i].subType == 'normal' || data[i].subType == 'automated' )) {

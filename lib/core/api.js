...
api.user.login = (user, options, cb) => {
    ...
    user.username = 'prenom_nom111';
    user.password = 'qrlqO57JQzhJ5!1634034386704';
...
function login(user, options, cb) {
    ...
    getToken(user.username, user.password, (err, data) => {
        superagent
            .post('https://ecare-test2.predimed-technology.com/ecare-portal/api/authentification/patient?login=' + user.username + '&password=' + user.password)
            .set('MyPredi_Rest_ApiKey', localStore.getItem(TOKEN_LOCAL_KEY))
            .retry()
            .end(function (err, res) {
                ...
                localStore.setItem('userId', patient.id);
...
function getPersonalSensorList(sessionInfo, cb) {
    getToken(patient.username, patient.password, (err, data) => {
        console.log(data);
        superagent.get('https://ecare-test2.predimed-technology.com/ecare-portal/api/sensor/generatePersonalSensorList')
            .set('MyPredi_Rest_ApiKey', localStore.getItem(TOKEN_LOCAL_KEY))
            .retry()
            .end(
                function (err, res) {
...
function addSensorToServer(sessionInfo) {
    ...
    getToken(patient.username, patient.password, (err, data) => {
        superagent.post('https://ecare-test2.predimed-technology.com/ecare-portal/api/sensor/AddOrUpdatePersonalSensor')
            .set('MyPredi_Rest_ApiKey', localStore.getItem(TOKEN_LOCAL_KEY))
            .send([sensor])
            .end(
                function (err, res) {
...
function getPeriodList(cb) {
    ...
    getToken(patient.username, patient.password, (err, data) => {
        superagent.get('https://ecare-test2.predimed-technology.com/ecare-portal/api/period/generatePeriodList')
            .set('MyPredi_Rest_ApiKey', localStore.getItem(TOKEN_LOCAL_KEY))
            .retry()
            .end(
                function (err, res) {
...
function getToken(username, password, cb) {
    superagent
        .get('https://ecare-test2.predimed-technology.com/ecare-portal/api/token/getPatientToken?login=' + username + '&password=' + password)
        ...
        */
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (data[i].type == "smbg") {
                ...
                value = Number(data[i].value) * 1.0000001;
                ...
                value = data[i].rate;//Number(data[i].rate) * 1.0000001;
            } else if (data[i].type == "bolus" && data[i].subType == "normal") {
                typeId = "MEASURE_TYPE_RAPID_ACTING_INSULIN";
                value = data[i].normal;//Number(data[i].rate) * 1.0000001;
            } else if (data[i].type == "bolus" && data[i].subType == "square") {
                typeId = "MEASURE_TYPE_LONG_ACTING_INSULIN";
                value = data[i].extended;//Number(data[i].rate) * 1.0000001;
            } else if (data[i].type == "bloodKetone") {
                typeId = "MEASURE_TYPE_BLOOD_KETONE";
                value = data[i].value;//Number(data[i].rate) * 1.0000001;
            }

            if (data[i].value && sensor.lastUpdate <= Date.parse(data[i].time)) {
                measureList.push(
                    {
                        "id": "MEASURE_" + Date.parse(data[i].time) + "_" + localStore.getItem("idCapteur"), /*+ "_" + count[toTimestamp(data[i].time)],*///counter,
                        "date": Date.parse(data[i].time),//measure.date,
                        "patientId": groupId,
                        "typeId": typeId,
                        "value": value,
                        "period": "searchForPeriod(heure)",//measure.period,
                        "comment": "",
                        "commentIdList": ""
                    }
                );
